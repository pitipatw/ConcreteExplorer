...
# Extract fc′ and embodied carbon datapoints
...
# filter = ec_Broyles .> 800; #deprecated
# deleteat!(dataset_Broyles, filter); #deprecated

# Note the psi and MPa version
fc′_Broyles_psi = dataset_Broyles[!,"Concrete Compressive Strength (psi)"];
ec_Broyles = dataset_Broyles[!, "A1-A3 Global Warming Potential (kg CO2-eq)"];
...
company_n_EPDs["Holcim - Aggregate Industries"]

line1 = Vector{Float64}(undef, 9);
line2 = Vector{Float64}(undef, 9);
line3 = Vector{Float64}(undef, 9);
line4 = Vector{Float64}(undef, 9);

# company_name = "National Ready Mix"
company_name = "Holcim - Aggregate Industries"
company_dataset = dataset_Broyles[dataset_Broyles.Company .== company_name, :]

n_label = combine((groupby(company_dataset, :Label, sort=true)), nrow)[!, :nrow]
...
println("If the compnay has 0, just output 0")
for i in 1:9
    ...
    company_dataset_label = company_dataset[company_dataset.Label .== i,"A1-A3 Global Warming Potential (kg CO2-eq)"]
...
boxplot!(ax4,fc′_design,ec_Broyles, color = colorant"#D3D3D3", show_outliers = false, show_median=false)

lines!(ax4, fc′_design,line2, label = "Maximum", linewidth = 3)
lines!(ax4, fc′_design,line1, label = "Median",  linewidth = 3)
lines!(ax4, fc′_design,line3, label = "Minimum", linewidth = 3)
