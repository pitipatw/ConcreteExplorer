...
function getMcr(material::Material, properties::Properties, Ω::Float64)
    @unpack fc′, Ec, Eps, fpy, fr = material
    @unpack Aps, Atr, Itr, Zb, em, w, moment_selfweight, fr, r, fpe = properties

    Mcre = Aps * fpe * (em + Zb / Atr) + (fr * Zb) # Cracking moment due to initial effective prestress (mcre)
    @assert Mcre > 0 "Error Mcre is less than 0."
    ΔMcr = (Aps * em * (em + Zb / Atr) * (Mcre - moment_selfweight)) / ((1 / Ω * Itr * Ec / Eps) + Aps * (r^2 - em * Zb / Atr)) # Moment due to stress increase in external tendons.
    @assert ΔMcr > 0 "Error ΔMce is less than 0."
...
"""function getΩ(properties::Properties)
equation (6)
"""
function getΩ(properties::Properties)
    @unpack em, es, Ls, Ld, L = properties
